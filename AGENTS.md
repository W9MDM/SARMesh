...
7. **Manifest-only fast path:** when the staged set contains nothing but `package.json`, `pnpm-lock.yaml`, and `net.nwimesh.SARMesh.yml`, the repo-wide source scanners and Vitest are skipped (`pre-commit: skip … (manifest-only commit)`); `typecheck`, `check:flatpak`, `check:licenses`, and `pnpm audit` still run. A staged `package.json` also re-runs `scripts/sync-flatpak-electron.mjs` and re-stages the manifest so the vendored Electron/pnpm pins self-heal. Mixed manifest + source commits keep the full pre-commit path.
