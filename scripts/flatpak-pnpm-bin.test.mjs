...
describe('Flatpak pnpm standalone install', () => {
  it('copies pnpm-vendor/dist beside the wrapper binary (pnpm 11+ needs dist/pnpm.mjs)', () => {
    ...
    expect(yaml).toMatch(
      /install -Dm755 pnpm-vendor\/pnpm \/run\/build\/sarmesh\/\.pnpm-bin\/pnpm/,
    );
    expect(yaml).toMatch(/cp -a pnpm-vendor\/dist \/run\/build\/sarmesh\/\.pnpm-bin\/dist/);
  });
  ...
  - name: sarmesh
    build-options:
      env:
        # PNPM_CONFIG_TRUST_LOCKFILE: 'true'
        # PNPM_CONFIG_VERIFY_DEPS_BEFORE_RUN: 'false'
        PNPM_HOME: /run/build/sarmesh/.pnpm
`;
