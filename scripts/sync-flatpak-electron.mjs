...
async function main() {
  ...
  if (checkOnly) {
    ...
    for (const { label, version } of stale) {
      console.error(
        `sync-flatpak-electron: net.nwimesh.SARMesh.yml is out of sync with ${label} ${version}`,
      );
    }
  ...
  for (const { label, version, changed } of results) {
    console.log(
      changed
        ? `sync-flatpak-electron: updated net.nwimesh.SARMesh.yml for ${label} ${version}`
        : `sync-flatpak-electron: net.nwimesh.SARMesh.yml already matches ${label} ${version}`,
    );
  }
}
