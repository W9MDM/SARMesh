#!/usr/bin/env node
/**
 * Write installer / package notice files when SARMESH_SCHEMA_BUMPED=1.
...
export function formatSchemaUpgradeNoticeText(opts) {
  ...
  return (
    `SARMesh database schema upgrade\n` +
    `====================================\n\n` +
    ...
    `You cannot downgrade to an older SARMesh that only supports the\n` +
    `previous schema while keeping the same database.\n\n` +
    `On first launch, SARMesh shows a Quit / Upgrade dialog before\n` +
...
export function formatNsisSchemaUpgradeInclude(message) {
  ...
  return (
    ...
    `!define SARMESH_SCHEMA_UPGRADE_NOTICE "${escaped}"\n`
  );
}
...
export function writeSchemaUpgradeNoticeFiles(env = process.env, resourcesDir = RESOURCES) {
  const bumped = env.SARMESH_SCHEMA_BUMPED === '1' || env.SARMESH_SCHEMA_BUMPED === 'true';
  ...
  const currSchema = Number(env.SARMESH_SCHEMA_CURR ?? '');
  const prevRaw = env.SARMESH_SCHEMA_PREV ?? '';
  const prevSchema = prevRaw !== '' && /^\d+$/.test(prevRaw) ? Number(prevRaw) : null;
  const prevTag =
    typeof env.SARMESH_SCHEMA_PREV_TAG === 'string' && env.SARMESH_SCHEMA_PREV_TAG
      ? env.SARMESH_SCHEMA_PREV_TAG
      : null;

  if (!Number.isInteger(currSchema) || currSchema < 1) {
    throw new Error('SARMESH_SCHEMA_CURR must be a positive integer when schema is bumped');
  }
