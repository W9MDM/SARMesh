...
describe('matchWinSetupInstallerArch', () => {
  it('matches default and stamped names', () => {
    expect(matchWinSetupInstallerArch('5.26.0', 'SARMesh Setup 5.26.0.exe')).toBe('x64');
    expect(matchWinSetupInstallerArch('5.26.0', 'SARMesh Setup 5.26.0-arm64.exe')).toBe('arm64');
    expect(matchWinSetupInstallerArch('5.26.0', 'SARMesh Setup 5.26.0-run214.exe')).toBe('x64');
    expect(matchWinSetupInstallerArch('5.26.0', 'SARMesh Setup 5.26.0-run214-arm64.exe')).toBe(
      'arm64',
    );
    expect(matchWinSetupInstallerArch('5.26.0', 'SARMesh-Setup-5.26.0.exe')).toBe('x64');
    expect(matchWinSetupInstallerArch('5.26.0', 'SARMesh-Setup-5.26.0-arm64.exe')).toBe('arm64');
  ...
  it('rejects wrong version and non-setup exes', () => {
    expect(matchWinSetupInstallerArch('5.26.0', 'SARMesh Setup 5.25.0.exe')).toBeNull();
    expect(matchWinSetupInstallerArch('5.26.0', 'SARMesh.exe')).toBeNull();
    expect(matchWinSetupInstallerArch('5.26.0', 'SARMesh Setup 5.26.0__uninstaller.exe')).toBe(
      null,
    );
  });
});
...
describe('collectWinSetupInstallers', () => {
  it('collects hyphenated pair', () => {
    expect(
      ...
      x64: 'SARMesh-Setup-5.26.0.exe',
      arm64: 'SARMesh-Setup-5.26.0-arm64.exe',
    });
  });
  ...
  it('collects stamped pair', () => {
    expect(
      ...
      x64: 'SARMesh Setup 5.26.0-run214.exe',
      arm64: 'SARMesh Setup 5.26.0-run214-arm64.exe',
    });
  });
  ...
  it('finds stamped arch', () => {
    expect(
      findWinSetupInstaller('5.26.0', 'arm64', [
        'SARMesh Setup 5.26.0-run9.exe',
        'SARMesh Setup 5.26.0-run9-arm64.exe',
      ]),
    ).toBe('SARMesh Setup 5.26.0-run9-arm64.exe');
  });
});

describe('hyphenateWinSetupInstallerName', () => {
  it('replaces spaces with hyphens', () => {
    expect(hyphenateWinSetupInstallerName('SARMesh Setup 5.36.0.exe')).toBe(
...
describe('hyphenatedWinSetupNameFromGithubDotted', () => {
  it('maps GitHub dotted Setup names to latest.yml urls', () => {
    expect(hyphenatedWinSetupNameFromGithubDotted('SARMesh.Setup.5.36.0.exe')).toBe(
      'SARMesh-Setup-5.36.0.exe',
    );
    expect(hyphenatedWinSetupNameFromGithubDotted('SARMesh.Setup.5.36.0-arm64.exe')).toBe(
      'SARMesh-Setup-5.36.0-arm64.exe',
    );
    expect(hyphenatedWinSetupNameFromGithubDotted('SARMesh-Setup-5.36.0.exe')).toBeNull();
  });
});
...
describe('normalizeWinSetupInstallerNames', () => {
  it('renames spaced Setup exes in a directory', () => {
    ...
    expect(fs.existsSync(path.join(root, 'SARMesh-Setup-5.36.0.exe'))).toBe(true);
    expect(fs.existsSync(path.join(root, 'SARMesh-Setup-5.36.0-arm64.exe'))).toBe(true);
  });
});
